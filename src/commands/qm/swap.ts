...
export default class QMSwap extends ExtendedCommand<typeof QMSwap> {
  ...
  public async run(): Promise<void> {
    ...
    if (current?.success) {
      this.log(deviceStatusHelper(`Swap - Power off QM ${current?.data?.id} (${current?.data?.name})`, chalk.magenta("queued"), { level: 2, isLast: false }));
    };
    this.log(deviceStatusHelper("Swap - Device Sync", chalk.magenta("queued"), { level: 2, isLast: false }));
    this.log(deviceStatusHelper(`Swap - Start QM ${swapper.id} (${swapper.friendlyName})`, chalk.magenta("queued"), { level: 2, isLast: true }));
  }
}
